# India + ATS / FAANG Resume Generation Prompt (Latest)

Use this prompt with any job description to generate a tailored **2-page ATS-friendly India-format resume** that also clears FAANG / big-company ATS parsers (Workday, Greenhouse, Lever, Taleo, iCIMS, SmartRecruiters, SuccessFactors, Ashby, and proprietary FAANG portals).

Your goal: produce a resume that (1) parses cleanly in ATS, (2) ranks well on JD keyword/skills matching, (3) passes a 30–60 second recruiter screen with quantified impact, and (4) includes all India hiring fields employers and consultants expect.

---

## INPUT (required)

1. **Target Job Description:** Paste the full JD (or `@jd_file` / `@target_jd`)
2. **Candidate Source of Truth:** Point to reference folder / past resumes / notes (e.g., `@resume_reference` or `@resume_referenece`)
...
5. **Output Format (default for this repo):** Decoupled JSON page structure → `generate_resume.py`
   ...
   - Set `meta.location_key` to `"india"` and use India logistics in `location` + `additional`
...
7. **Optional India corporate fields flag:** State whether to include CTC / joining logistics / employment-type details (default: **omit** unless JD or user explicitly requests)

---

## RESUME REQUIREMENTS (INDIA + ATS)

1. **Format:** Exactly **two pages**, A4, ATS-friendly India-format resume
2. **Layout:** Single-column only — clean sections, no graphics/icons/skill bars/photos/logos
3. **India-specific fields (required in header and/or Additional Information):**
   - Current city
   - Preferred work locations (India)
   - Notice period
   - Work authorization for India
   - Remote / hybrid / onsite preference
   - Languages and travel readiness (as relevant)
4. **Header (body, not PDF header/footer):** Full name, IC/senior title, email, phones (India first), location lines that **fit without wrapping orphans**, nationality/auth, notice. Work mode belongs in Additional Information, not crammed into the location line.
...
- **India employers / IT services / product companies:** Same enterprise ATS family; keyword match + India logistics fields (notice period, locations, work mode) for human screeners and staffing partners
...
3. **No headers/footers for contact info** — contact + India fields in the document body at the top
...
## MANDATORY SECTION ORDER (INDIA HYBRID + ATS)

Use this exact order and these exact headings:

1. **Contact Header** (no section title) — includes India logistics fields
2. **Professional Summary**
3. **Technical Skills**
4. **Certifications** (prominent before experience — India preference)
5. **Professional Experience**
6. **Awards & Recognition** (if any)
7. **Career Impact Summary** (plain-text metrics block; ATS-safe)
8. **Education**
9. **Additional Information** (India fields + languages + travel readiness)
...
4. **Quantified impact:** Most bullets should carry measurable results (percentages, **rupee impact where applicable**, dollar amounts if global delivery, scale numbers, latency, uptime, MTTR, deploy frequency, cost). Max **two metrics per bullet**; where the source has no number, use a scale/scope proof instead of inventing one
...
| **Y** | Proof | One number, %, ₹/$, scale, SLA, or data source | Restating the same % already used as X; the how |
...
- Two phones: label by market. India-target resume: India number first (`+91 … | +357 …`). Dubai-target: local/reachable number first.
...
### 7) India logistics (this prompt)
- Header must make the India move obvious **without wrapping or duplicated labels** (see Contact Header field map).
- If still abroad: `relocation_line` = `Relocating to India`. Do **not** put relocating inside `current_location`.
- Additional Information carries work mode, metro list (if not already in Target), and repeats notice + auth for staffing-partner grep.
...
5. **No framework labels:** Framework names (Google XYZ, STAR, LPS, ELITE) must **never** appear in the resume text / `resume_data.json` / PDF content
...
### 1) Contact Header
`generate_resume.py` builds the header from `header` + `location`. Do **not** invent extra header sentences. Map fields as follows so each rendered line stays on **one line** (no orphan last word like `metros`).

**Rendered layout (exactly this shape):**
```
PRASHAM TATED
SENIOR DEVOPS ENGINEER  |  PLATFORM ENGINEERING  |  SRE
+91 …  |  +357 …  |  email  |  LinkedIn  |  GitHub  |  portfolio
Based in {current_location}  |  Target: {target_location}  |  {relocation_line}
{authorization}  |  Availability: {availability}
```

**`location` JSON (copy this contract):**
| Field | Write this | Do not write this |
|------|------------|-------------------|
| `phone` | `+91 9890492331  \|  +357 95910171` (India first) | A third number; unlabeled soup |
| `current_location` | `Limassol, Cyprus` | `Limassol, Cyprus (relocating to India)` — relocating is a separate field |
| `target_location` | `Pune / Mumbai / Bangalore / Hyderabad` | Extra clauses (`open to metros`, work mode) |
| `relocation_line` | `Relocating to India` (omit if already in India) | `Onsite, hybrid, or remote in India metros` — too long; wraps |
| `availability` | `Immediate to 30 days` | `Notice: immediate to 30 days` — the generator already prefixes `Availability:` |
| `authorization` | `Indian national  \|  No India work authorization required` | Visa essays |

**Hard rules:**
- Name, title, contact, location line, logistics line = 5 header rows. Title may wrap to 2 lines only if the JD title is long.
- Location line and logistics line must each fit on **one line** at 9pt. If a draft wraps (`metros` alone, `days` alone), shorten `relocation_line` / `target_location` / `authorization`.
- Work mode (onsite / hybrid / remote) goes in **Additional Information**, not in `relocation_line`.
- No `Availability: Notice:` stacking. No duplicated `relocating` in both current_location and relocation_line.
...
- India / international signal (relocating, notice, EU delivery) without repeating every logistics field
...
- Examples: Deployment frequency, incident reduction, cost savings (₹ where applicable), uptime/SLO, platform scale
...
### 9) Additional Information (India)
Include:
- India move / preferred metros (if not fully visible in the one-line Target field)
- Notice period + work authorization
- **Work mode** (Onsite, Hybrid, or Remote) — required here because it is **not** in the header location line
- Languages
- Travel readiness
- Total experience / relevant experience (optional short line if helpful for Indian staffing filters)
...
## OPTIONAL INDIA CORPORATE FIELDS

Include **only if** requested in the JD or explicitly asked by the user:

1. **Compensation details:** Current CTC, expected CTC, and negotiability
2. **Joining logistics:** Earliest joining date and interview availability windows
3. **Employment details:** Current employer type (product / services / consulting), total experience, relevant experience, and career break explanation (if any)

If not requested, **omit entirely** — do not add salary/CTC by default (also aligns with FAANG/ATS “never include salary unless required” rule).
...
3. Prefer exact phrase matches from the JD (critical for Taleo / strict keyword ATS and many India enterprise portals)
...
Optimize for signals big-tech and top India product/enterprise employers scan for:
...
- Header orphans / wrap (`metros` on its own line) or stacked labels (`Availability: Notice:`)
- Relocating stuffed into `current_location` or work mode stuffed into `relocation_line`
...
- More than 2 pages (or less than 2 — target exactly 2 for India format)
...
1. Create/update company folder: `<company_slug>/` (e.g., `trianz/`, `optum/`)
2. Write JD-tailored content to `<company_slug>/resume_data.json` using the schema in `templates/RESUME_DATA_SCHEMA.md`. **Start from `master_resume_india.json`** (recruiter-fixed master). Do not start from `templates/default_india.json` unless the master file is missing.
3. Ensure `meta.location_key` is `"india"` and `location` follows the Contact Header field map (`current_location` city only, short `relocation_line`, `availability` without a Notice prefix, India phone first)
4. Set `key_metrics` to `[]` unless the user explicitly wants the banner. Empty list = no banner in `generate_resume.py`.
4. Render PDF into that folder:
   ```bash
   source .venv/bin/activate
   python generate_resume.py --data <company_slug>/resume_data.json
   ```
5. If the generator exits with a **>2 pages** error, shorten summary / skills / bullets in the JSON and re-run until exactly 2 pages
6. Keep the JD file in the same folder (e.g., `trianz/jd`) for traceability
7. Include CTC / joining / employment-type fields in `additional` **only** when the JD or user explicitly requests them
...
2. **ATS Checklist** (pass/fail):
   - Single column
   - Standard headings
   - Contact + India fields in body (not header/footer)
   - No repeated header or footer on page 2
   - No layout tables/images/icons
   - Date consistency
   - JD required-keyword coverage list (Present / Missing)
3. **India Fields Checklist:** current city, relocating-to-India if abroad, preferred locations, notice period, work authorization, remote/hybrid/onsite
4. **Optional Corporate Fields Note:** state whether CTC / joining / employment-type fields were included and why
5. **Tailoring Notes:** 5–8 bullets on what was emphasized for this JD
6. **Recruiter Scan Test:** 3 strongest impact lines a recruiter should notice in 30 seconds (must be on page 1)

---

## QUALITY GATE (MUST PASS BEFORE FINALIZING)

- [ ] Exactly **two pages** (India format) — adjust spacing to fit, not more, not less
- [ ] Single-column reverse-chronological hybrid with standard headings
- [ ] **Header layout:** location line and logistics line each fit on one line (no orphan wrap); `availability` has no `Notice:` prefix; relocating is only in `relocation_line`; work mode is in Additional Information
- [ ] India fields present (city, preferred locations, notice period, work authorization, work mode)
- [ ] All major JD required keywords appear at least once (naturally)
- [ ] Every recent-role bullet has impact/metric where data exists (₹ where applicable)
...
- [ ] India move is obvious (relocating if abroad, metros, notice, Indian national)
...
- [ ] CTC/salary omitted unless explicitly requested
- [ ] Summary + Skills front-load the highest-weight keywords
- [ ] Content is truthful to candidate source materials
- [ ] India-market relevance: notice period, location preference, and work authorization clearly stated
- [ ] Content lives in `resume_data.json` (not hardcoded in `generate_resume.py`)
- [ ] PDF rendered via `python generate_resume.py --data <company_slug>/resume_data.json`
